...
#define system_canopen_set_bitrate(canopen, value) actor_set_property_numeric(canopen->actor, (uint32_t) value, sizeof(uint16_t), SYSTEM_CANOPEN_BITRATE)
#define system_canopen_get_bitrate(canopen) *((uint16_t *) actor_get_property_pointer(canopen->actor, &(uint8_t[sizeof(uint16_t)]{}), sizeof(uint16_t), SYSTEM_CANOPEN_BITRATE)
