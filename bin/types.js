...
od.replace(/\{\s*([^}]+?)\s*\}[^}]+?x([3-9].*?)_([a-z]+)([A-Z][^_\s,;]+)/g, (match, struct, index, type, name) => {
  ...
  struct.split(/;/).map((pair, attributeIndex) => {
    ...
    var hex = `0x${(toHex(attributeIndex + 1))}`
    defs.push(`${constant} = ${hex}`);
    ...
    var comment = `/* 0x${attributeOD}: ${attributeDefinition.description} */\n`

    
    if (dataType == 'char') {
      accessors.push(`${comment}#define ${type}_${name}_get_${attribute}(actor) ((${dataType} *) actor_property_pointer_get(actor_of_type(actor, 0x${index}), ${hex}))`);
    } else {
      accessors.push(`${comment}#define ${type}_${name}_get_${attribute}(actor) *((${dataType} *) actor_property_pointer_get(actor_of_type(actor, 0x${index}), ${hex}))`);
    } 
    ...
    if (foundPhase) {
      ...
      if (arrayLength) {
        accessors.push(`${comment}#define ${type}_${name}_set_${attribute}(actor, value, size) actor_property${dataType == 'char' ? '_string_set' : '_set'}(actor_of_type(actor, 0x${index}), ${hex}, value, size)`);
//        accessors.push(`/* 0x${attributeOD}: ${attributeDefinition.description} */
//static inline void ${type}_${name}_set_${attribute}(actor_t *${name}, ${dataType} *value, size_t size) {
//    actor_property_set${dataType == 'char' ? '_string' : ''}(${name}, ${constant}, value, size);
//}`);
      } else {
//        accessors.push(`/* 0x${attributeOD}: ${attributeDefinition.description} */
//static inline void ${type}_${name}_set_${attribute}(actor_t *${name}, ${dataType} value) { 
//    actor_property_numeric_set(${name}, ${constant}, (uint32_t)(value), sizeof(${dataType}));
//}`);
        accessors.push(`${comment}#define ${type}_${name}_set_${attribute}(actor, value) actor_property_numeric_set(actor_of_type(actor, 0x${index}), ${hex}, (uint32_t) (value), sizeof(${dataType}))`);
      }
    }
    /*
    if (dataType == 'char') {
      accessors.push(`static inline ${dataType} *${type}_${name}_get_${attribute}(actor_t *${name}) {
    return (${dataType} *) actor_property_pointer_get(${name}, ${constant});
}`);
    } else {
      accessors.push(`static inline ${dataType} ${type}_${name}_get_${attribute}(actor_t *${name}) {
    return *((${dataType} *) actor_property_pointer_get(${name}, ${constant}));
