...
static actor_signal_t sdio_signal_received(transport_sdio_t *sdio, actor_signal_t signal, actor_t *caller, void *source) {
    switch (signal) {
...
static actor_signal_t sdio_task_erase_block(actor_job_t *job, actor_signal_t signal, actor_t *caller, uint32_t block_id,
...
static actor_signal_t sdio_task_write_block(actor_job_t *job, actor_signal_t signal, actor_t *caller, uint32_t block_id, uint8_t *data,
                                            uint32_t block_count) {
    transport_sdio_t *sdio = job->actor->object;
    actor_t *sdcard = job->inciting_message.producer;
...
static actor_signal_t sdio_task_read_block(actor_job_t *job, actor_signal_t signal, actor_t *caller, uint32_t block_id, uint8_t *data,
                                           uint32_t block_count) {
    transport_sdio_t *sdio = job->actor->object;
    actor_t *sdcard = job->inciting_message.producer;
    uint32_t block_address = storage_sdcard_get_high_capacity(sdcard) ? block_id : block_id * 512;
...
static actor_signal_t sdio_task_detect_card(actor_job_t *job, actor_signal_t signal, actor_t *caller) {
    transport_sdio_t *sdio = job->actor->object;
    actor_t *sdcard = job->inciting_message.producer;
    // Power up the card
    switch (job->task_phase) {
...
static actor_signal_t sdio_task_publish_response(actor_job_t *job, actor_signal_t signal, actor_t *caller) {
    transport_sdio_t *sdio = job->actor->object;
    switch (job->task_phase) {
    case 0:
        // if event was ACTOR_MESSAGE_READ_TO_BUFFER, it is assumed that producer expects report instead of event
...
static actor_signal_t sdio_job_read(actor_job_t *job, actor_signal_t signal, actor_t *caller) {
    switch (job->job_phase) {
    case 0:
        return sdio_task_read_block(job, signal, caller, (uint32_t)job->inciting_message.argument, job->inciting_message.data,
...
static actor_signal_t sdio_job_erase(actor_job_t *job, actor_signal_t signal, actor_t *caller) {
    switch (job->job_phase) {
    case 0:
        return sdio_task_erase_block(job, signal, caller, (uint32_t)job->inciting_message.argument, job->inciting_message.size);
    case 1:
        return ACTOR_SIGNAL_TASK_COMPLETE;
    }
    return signal;
...
static actor_signal_t sdio_job_write(actor_job_t *job, actor_signal_t signal, actor_t *caller) {
    ...
    case 1:
        return sdio_task_write_block(job, signal, caller, (uint32_t)job->inciting_message.argument, job->inciting_message.data,
                                     job->inciting_message.size);
    case 2:
        return ACTOR_SIGNAL_TASK_COMPLETE;
    }
...
static actor_signal_t sdio_on_high_priority(transport_sdio_t *sdio, actor_message_t *event, actor_worker_t *tick, actor_thread_t *thread) {
    return actor_job_execute_if_running_in_thread(sdio->job, thread);
}

static actor_signal_t sdio_on_input(transport_sdio_t *sdio, actor_message_t *event, actor_worker_t *tick, actor_thread_t *thread) {
    switch (event->type) {
    case ACTOR_MESSAGE_READ:
    case ACTOR_MESSAGE_READ_TO_BUFFER:
        return actor_message_handle_and_start_job(sdio->actor, event, &sdio->job, sdio->actor->node->high_priority, sdio_job_read);
    case ACTOR_MESSAGE_WRITE:
        return actor_message_handle_and_start_job(sdio->actor, event, &sdio->job, sdio->actor->node->high_priority, sdio_job_write);
    case ACTOR_MESSAGE_ERASE:
        return actor_message_handle_and_start_job(sdio->actor, event, &sdio->job, sdio->actor->node->high_priority, sdio_job_erase);
    case ACTOR_MESSAGE_MOUNT:
        return actor_message_handle_and_start_job(sdio->actor, event, &sdio->job, sdio->actor->node->high_priority, sdio_job_mount);
    case ACTOR_MESSAGE_UNMOUNT:
        return actor_message_handle_and_start_job(sdio->actor, event, &sdio->job, sdio->actor->node->high_priority, sdio_job_unmount);
    default:
        break;
    }
    return 0;
...
static actor_worker_callback_t sdio_worker_assignment(transport_sdio_t *sdio, actor_thread_t *thread) {
