...
static actor_signal_t modbus_signal(transport_modbus_t *modbus, actor_signal_t signal, actor_t *caller, char *source) {
    switch (signal) {
        /* usart is idle, need to wait 3.5 characters to start reading */
        case ACTOR_SIGNAL_RX_COMPLETE:
            actor_send(modbus->timer, modbus->actor, modbus->idle_timeout, ACTOR_SIGNAL_RX_COMPLETE);
            break;
...
int transport_modbus_read(transport_modbus_t *modbus, uint8_t *data) {
    ...
    // finish with request
    actor_set_phase(modbus->actor, ACTOR_PHASE_OPERATION);
    return 0;
}
...
int transport_modbus_request(transport_modbus_t *modbus, uint8_t recipient, uint16_t index, uint8_t type, uint16_t length,
    ...
    if (actor_get_phase(modbus->actor) != ACTOR_PHASE_OPERATION) {
        return 1;
    }
...
actor_interface_t transport_modbus_class = {
    ...
    .construct = modbus_construct,
    .link =  modbus_link,
    .destruct =  modbus_destruct,
    .start =  modbus_start,
    .stop =  modbus_stop,
    .signal_received = (actor_signal_received_t) modbus_signal,
    .phase_changed = (actor_phase_changed_t)  modbus_phase,
