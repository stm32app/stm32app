...
// Software trigger
    ...
    // actor_set_temporary_phase(adc->actor, ACTOR_PHASE_PREPARATION, 10000);

    actor_set_phase(adc->actor, ACTOR_PHASE_OPERATION);
    return 0;
}
...
static actor_signal_t adc_phase_changed(module_adc_t *adc, actor_phase_t phase) {
    switch (phase) {
    case ACTOR_PHASE_START:
        return adc_start(adc);
    case ACTOR_PHASE_STOP:
        return adc_stop(adc);
    case ACTOR_PHASE_CONSTRUCTION:
        return adc_construct(adc);
    case ACTOR_PHASE_DESTRUCTION:
        return adc_destruct(adc);
    default:
        break;
    }
    return 0;
...
static actor_signal_t  adc_signal_received(module_adc_t *adc, actor_signal_t signal, actor_t *caller, void *arg) {
    switch (signal) {
        case ACTOR_SIGNAL_LINK:
            return adc_accept(adc, caller, arg);
        default:
            break;
    }
    return 0;
...
actor_interface_t module_adc_class = {
    .type = MODULE_ADC,
    .size = sizeof(module_adc_t),
    .phase_subindex = MODULE_ADC_PHASE,
    .validate = (actor_method_t) adc_validate,
    .value_received = (actor_value_received_t) adc_receive,
    .signal_received = (actor_signal_received_t) adc_signal_received,
    .phase_changed = (actor_phase_changed_t) adc_phase_changed,
};
