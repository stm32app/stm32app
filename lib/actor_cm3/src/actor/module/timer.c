...
static actor_signal_t timer_construct(module_timer_t *timer) {
    timer->subscriptions =
        actor_calloc_region(ACTOR_REGION_FAST, timer->properties->initial_subscriptions_count, sizeof(module_timer_subscription_t));
...
#endif
    default:
        error_printf("TIM%i is not supported by this actor", timer->actor->seq + 1);
    }
    return 0;
}
...
static actor_signal_t timer_schedule(module_timer_t *timer, uint32_t next_time) {
    if (timer->next_time > next_time) {
        timer->next_time = time_not_zero(next_time);
    }
    ...
    for (size_t i = 0; i < timer->properties->initial_subscriptions_count; i++) {
        ...
        if (diff <= 0) {
            ...
            *subscription = (module_timer_subscription_t){};
            actor_signal(actor, ACTOR_SIGNAL_TIMEOUT, timer->actor, argument);
        }
    }
    return 0;
}
...
static actor_signal_t timer_advance(module_timer_t *timer, uint32_t time) {
    ...
    if (actor_get_phase(timer->actor) != ACTOR_PHASE_OPERATION) {
        actor_set_phase(timer->actor, ACTOR_PHASE_OPERATION);
    }
    ...
    if (timer->next_time <= timer->current_time) {
        ...
        if (next == NULL) {
            actor_set_phase(timer->actor, ACTOR_PHASE_IDLE);
            return 0;
        } else {
            timer->next_time = time_not_zero(next->time);
        }
    }
...
static actor_signal_t timer_timeout(module_timer_t *timer, actor_t *actor, void *argument, uint32_t timeout) {
    ...
    bool was_next = timer->next_time == 0 || timer->next_time == subscription->time;
...
static actor_signal_t timer_value_received(module_timer_t *timer, actor_t *origin, uint32_t timeout, void *argument) {
    return timer_timeout(timer, origin, argument, timeout);
}
...
static actor_signal_t timer_phase_changed(module_timer_t *timer, actor_phase_t phase) {
    switch (phase) {
    ...
    case ACTOR_PHASE_CONSTRUCTION:
        return timer_construct(timer);
    case ACTOR_PHASE_OPERATION:
        return timer_start_counting(timer);
        break;
    case ACTOR_PHASE_IDLE:
        return timer_stop_counting(timer);
        break;
    default:
        break;
...
actor_interface_t module_timer_class = {
    .type = MODULE_TIMER,
    .size = sizeof(module_timer_t),
    .phase_subindex = MODULE_TIMER_PHASE,
    .validate = (actor_method_t)timer_validate,
    .phase_changed = (actor_phase_changed_t)timer_phase_changed,
    .value_received = (actor_value_received_t)timer_value_received,
    .signal_received = (actor_signal_received_t)timer_signal_received,
};
