...
static inline void module_adc_set_phase(module_adc_t *adc, uint8_t value) { 
    actor_set_property_numeric(adc->actor, MODULE_ADC_PHASE, (uint32_t)(value), sizeof(uint8_t));
}
static inline uint8_t module_adc_get_phase(module_adc_t *adc) {
    return *((uint8_t *) actor_get_property_pointer(adc->actor, MODULE_ADC_PHASE));
}
