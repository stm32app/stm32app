...
void fsmc_async_wait_disable(uint32_t bcr) {
    FSMC_BCR(bcr) &= ~FSMC_BCR_ASYNCWAIT;
}
void fsmc_async_wait_enable(uint32_t bcr) {
    FSMC_BCR(bcr) |= FSMC_BCR_ASYNCWAIT;
}
...
void fsmc_set_memory_width(uint32_t bcr, uint32_t width) {
    FSMC_BCR(bcr) |= width;
}
