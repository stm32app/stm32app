...
void actors_dma_notify(uint8_t unit, uint8_t index) {
    ...
    if (dma_get_interrupt_flag(dma_get_address(unit), index, DMA_TEIF | DMA_DMEIF | DMA_FEIF)) {
        ...
        if (actor->interface->signal_received(actor->object, ACTOR_SIGNAL_DMA_ERROR, NULL, actor_dma_pack_source(unit, index))) {
        ...
        if (actor->interface->signal_received(actor->object, ACTOR_SIGNAL_DMA_TRANSFERRING, NULL, actor_dma_pack_source(unit, index)) == 0) {
