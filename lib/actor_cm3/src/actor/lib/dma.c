...
actor_t* actors_dma[DMA_BUFFER_SIZE];

actor_t* actor_dma_get(uint8_t unit, uint8_t index) {
  return actors_dma[DMA_INDEX(unit, index)];
};

actor_signal_t actor_dma_acquire(actor_t* actor, uint8_t unit, uint8_t index) {
  actor_t* current = actors_dma[DMA_INDEX(unit, index)];
  if (current == actor) {
...
actor_signal_t actor_dma_release(actor_t* actor, uint8_t unit, uint8_t index) {
  actor_t* current = actors_dma[DMA_INDEX(unit, index)];
  if (!current) {
    return ACTOR_SIGNAL_UNAFFECTED;
  }
  if (current != actor) {
    return ACTOR_SIGNAL_CONFLICT;
  }
...
void actors_dma_notify(uint8_t unit, uint8_t index) {
  debug_printf("> DMA%i interrupt:\t\t%u left\n", unit,
               dma_get_number_of_data(dma_get_address(unit), index));
  volatile actor_t* actor = actor_dma_get(unit, index);
  ...
  if (dma_get_interrupt_flag(dma_get_address(unit), index, DMA_TEIF | DMA_DMEIF | DMA_FEIF)) {
    ...
    if (actor->interface->signal_received(actor->object, ACTOR_SIGNAL_DMA_ERROR, NULL,
                                          actor_dma_pack_source(unit, index))) {
    ...
    if (actor->interface->signal_received(actor->object, ACTOR_SIGNAL_DMA_TRANSFERRING, NULL,
                                          actor_dma_pack_source(unit, index)) == 0) {
...
// memory location of a buffer has to be aligned to burst_size * byte_width to ensure single AHB
// copy does cross 1kb boundary buffers allocated with `actor_buffer_aligned` are guaranteed to be
// aligned, while others may be aligned by accident
uint32_t actor_dma_get_safe_burst_size(uint8_t* data,
                                       size_t size,
                                       uint8_t width,
  ...
  uint8_t alignment =
      start_page_address == end_page_address ? 16 : get_maximum_byte_alignment((uint32_t)&data, 16);
...
void actor_dma_rx_start(uint32_t periphery_address,
                        uint8_t unit,
                        uint8_t stream,
                        uint8_t channel,
                        uint8_t* data,
...
void actor_dma_tx_start(uint32_t periphery_address,
                        uint8_t unit,
                        uint8_t stream,
                        uint8_t channel,
                        uint8_t* data,
