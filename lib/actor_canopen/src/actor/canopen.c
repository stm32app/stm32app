...
static actor_signal_t canopen_worker_high_priority(system_canopen_t *canopen, void *argument, actor_worker_t *tick, actor_thread_t *thread) {
    ...
    if (us_until_next != (uint32_t)-1) {
        actor_thread_worker_schedule(thread, tick, thread->current_time + us_until_next / 1000);
    }
    return 0;
}
...
actor_class_t system_canopen_class = {
    ...
    .destruct = (actor_method_t)canopen_destruct,
    .link = (actor_method_t)canopen_link,
    .start = (actor_method_t)canopen_start,
    .stop = (actor_method_t)canopen_stop,
