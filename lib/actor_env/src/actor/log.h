...
#define debug_timestamp_printf(...) \
  (log_ccycnt_before(), conditional_printf(__VA_ARGS__), log_ccycnt_after())
#else
...
// int)uiSize) #define traceFREE(pvAddress, uiSize)    debug_printf("%p %d free\r\n", pvAddress,
// (unsigned int)uiSize)
