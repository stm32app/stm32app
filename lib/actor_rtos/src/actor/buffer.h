...
/* Buffer wraps a growable chunk of memory that can be filled with long messages. Buffer is owned by
one of the actors that is responsible for growing and deallocating the memory. Buffers are meant to
be passed between actors: 1) With full ownership transfer: ensuring memory hand-off without overhead
of data copy. 2) By claiming the use of the buffer, making the owner retain the memory until all
users will release the buffer.
*/
struct actor_buffer {
...
// Get growable buffer of specified `size` *to write to*, optionally using given `data` pointer as
// initial allocation, accepts `options`
actor_buffer_t* actor_buffer_create_with_options(actor_t* owner,
                                                 uint8_t* data,
                                                 uint32_t size,
                                                 uint8_t options);
// Get an empty buffer of given `size` *to write to*, either pointing to given `data` or freshly
// allocated
#define actor_buffer_create(owner, data, size) \
  actor_buffer_create_with_options(owner, data, size, 0)
...
// Get growable buffer *to write to* with freshly allocated memory of specified `size`, accepts
// `options`
#define actor_buffer_empty_with_options(owner, size, options) \
  actor_buffer_create_with_options(owner, NULL, size, options)
...
// Get an empty buffer of given `size` *to write to*, either pointing to given `data` or freshly
// allocated When `size` is `-1`, it assumes data points to another buffer that can be used directly
// with reference tracking Accepts `options`
actor_buffer_t* actor_buffer_target_with_options(actor_t* owner,
                                                 uint8_t* data,
                                                 uint32_t size,
                                                 uint8_t options);
// Get an empty buffer of given `size` *to write to*, either pointing to given `data` or freshly
...
#define actor_buffer_target(owner, data, size) \
  actor_buffer_target_with_options(owner, data, size, 0)
#define actor_buffer_target_aligned(owner, data, size, alignment) \
  (data == NULL ? actor_buffer_aligned(owner, size, 16)           \
                : actor_buffer_target_with_options(owner, data, size, 0))

// Allocate an empty oversized buffer *to write to*, with shifted data pointer matching byte
// `alignment`, accepts options
actor_buffer_t* actor_buffer_aligned_with_options(actor_t* owner,
                                                  uint32_t size,
                                                  uint8_t options,
                                                  uint8_t alignment);
// Allocate an empty oversized buffer *to write to*, with shifted data pointer matching byte
// `alignment`
#define actor_buffer_aligned(owner, size, alignment) \
  actor_buffer_aligned_with_options(owner, size, alignment, 0)

// Wrap given memory into a buffer *to read from*, so its interal `size` will equal given `size`
// Size of -1 indicates that `data` points to another buffer that can be used directly with
// reference tracking Accepts `options`
actor_buffer_t* actor_buffer_source_with_options(actor_t* owner,
                                                 uint8_t* data,
                                                 uint32_t size,
                                                 uint8_t options);
// Wrap given memory into a buffer *to read from*, so its interal `size` will equal given `size`
// Size of -1 indicates that `data` points to another buffer that can be used directly with
// reference tracking
#define actor_buffer_source(owner, data, size) \
  actor_buffer_source_with_options(owner, data, size, 0)
...
// Accepts options
actor_buffer_t* actor_buffer_snapshot_with_options(actor_t* owner,
                                                   uint8_t* data,
                                                   uint32_t size,
                                                   uint8_t options);
// Copy given memory into a new buffer *to read from*, its internal `size` will equal given `size`
// Size of -1 indicates that `data` points to another buffer, but it will not be referenced directly
#define actor_buffer_snapshot(owner, data, size) \
  actor_buffer_snapshot_with_options(owner, data, size, 0)
...
// Copy data from circular buffer to a growable buffer between `previous_position` and `position`,
// handling wraparound at `ring_buffer_size`
actor_signal_t actor_buffer_read_from_ring_buffer(actor_buffer_t* buffer,
                                                  uint8_t* ring_buffer,
...
actor_signal_t actor_buffer_reserve_with_limits(actor_buffer_t* buffer,
                                                uint32_t size,
...
actor_signal_t actor_buffer_write(actor_buffer_t* buffer,
                                  uint32_t position,
                                  uint8_t* data,
                                  uint32_t size);
actor_signal_t actor_buffer_append(actor_buffer_t* buffer, uint8_t* data, uint32_t size);

#define actor_double_buffer_get_input_size(back, front) \
  actor_double_buffer_get_input(back, front)->allocated_size
...
/*
  Double buffer is used when receiving data of unknown length externally (e.g. via DMA). It is a
  combination of circular and growable buffer. Back buffer is filled continuously wrapping around,
  every time the end of allocated memory is reached. Data is being copied from circular buffer to
  growable front buffer periodically (triggered externally/manually, e.g. on half/complete transfer
  interrupts). Front buffer will grow and reallocate on demand. Using two buffers incurs overhead
  for memory copy, so the following helpers will only use it when necessary.
*/

/* Initialize buffers for reception of data of unknown size.
  - If destination memory chunk is provided, it will be used by front buffer but it will NOT ever be
  freed by it
  - Both buffers will be used if `expected_size` is set to zero which indicates unknown size.
  - Expected size of `-1` indicates that `destination` points to a `actor_buffer_t` that can be used
  as `front`
*/
actor_buffer_t* actor_double_buffer_target(actor_buffer_t* back,
                                           actor_t* owner,
                                           uint8_t* data,
                                           uint32_t size);
// Release the locks to front and back buffers to make double buffer ready for next transaction
actor_signal_t actor_double_buffer_dereference(actor_buffer_t* back,
                                               actor_buffer_t* front,
                                               actor_t* owner);
// Release the locks, clear pointer to front buffer and return it
actor_buffer_t* actor_double_buffer_detach(actor_buffer_t* back,
                                           actor_buffer_t** front,
                                           actor_t* owner);

actor_signal_t actor_double_buffer_ingest_external_write(actor_buffer_t* back,
                                                         actor_buffer_t* front,
                                                         uint16_t position);

uint16_t actor_double_buffer_get_sufficent_buffer_size(actor_double_buffer_t* read);
actor_signal_t actor_double_buffer_stop(actor_buffer_t* back,
                                        actor_buffer_t* front,
                                        uint8_t** data,
                                        uint32_t* size);
void actor_double_buffer_from_ring_buffer(actor_double_buffer_t* read, uint16_t position);
#define actor_double_buffer_targets_ring_buffer(front, back) \
  actor_double_buffer_get_output(front, back) == front
