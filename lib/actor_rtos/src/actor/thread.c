...
static actor_signal_t actor_thread_message_actor_dispatch(actor_thread_t *thread, actor_message_t *event, actor_t *actor,
                                                        actor_worker_t *worker);
static size_t actor_thread_message_requeue(actor_thread_t *thread, actor_message_t *event, actor_message_status_t previous_status);
static inline bool actor_thread_message_queue_shift(actor_thread_t *thread, actor_message_t *event, size_t deferred_count);
...
void actor_thread_execute(actor_thread_t *thread) {
    ...
    actor_message_t event = {.producer = actor_thread_get_root_actor(thread), .type = ACTOR_MESSAGE_THREAD_ALARM};
...
*/
static inline bool actor_thread_should_notify_actor(actor_thread_t *thread, actor_message_t *event, actor_t *actor, actor_worker_t *worker) {
    switch (event->type) {
...
static actor_signal_t actor_thread_message_actor_dispatch(actor_thread_t *thread, actor_message_t *event, actor_t *actor,
    ...
    if (actor_thread_should_notify_actor(thread, event, actor, worker)) {
        debug_printf("├ %-22s#%s from %s\n", actor_stringify(actor), actor_message_stringify(event), actor_stringify(event->producer));
    ...
    case ACTOR_MESSAGE_ADDRESSED:
        ...
        if (queue != NULL) {
            ...
            if (xQueueSend(queue, event, 0)) {
                if (event->status == ACTOR_MESSAGE_DEFERRED && previous_status != ACTOR_MESSAGE_DEFERRED) {
                    return 1;
                }
            } else {
...
actor_signal_t actor_thread_message_await(actor_thread_t *thread, actor_message_t *event) {
    ...
    for (bool stop = false; stop == false;) {
        ...
        case ACTOR_SIGNAL_OK:
            ...
            *event = (actor_message_t){.producer = actor_thread_get_root_actor(thread), .type = ACTOR_MESSAGE_THREAD_ALARM};
...
bool actor_thread_publish_generic(actor_thread_t *thread, actor_message_t *event, bool to_front) {
    debug_printf("│ │ ├ Publish\t\t#%s to %s for %s on %s\n", actor_message_stringify(event), actor_stringify(event->producer),
    ...
    if (actor_thread_is_interrupted(thread)) {
        ...
        if (thread->queue == NULL || xQueueGenericSendFromISR(thread->queue, event, &xHigherPriorityTaskWoken, to_front)) {
            result = xTaskNotifyFromISR(thread->task, (uintptr_t)event, eSetValueWithOverwrite, &xHigherPriorityTaskWoken);
        };
        portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
    } else {
    ...
    if (thread != NULL) {
        ...
        *thread = (actor_thread_t){.actor = actor, .flags = flags};
        ...
        if (thread->worker_count > 0) {
            thread->workers = actor_calloc_region(ACTOR_REGION_FAST, thread->worker_count, sizeof(actor_worker_t));
