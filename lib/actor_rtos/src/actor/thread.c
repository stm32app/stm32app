...
/* A generic RTOS task function that handles all typical propertiesurations of threads. It supports following features or combinations of
  ...
  An node has at least five of these threads with different priorities. Devices declare "workers" which act as methods for a corresponding
  ...
  longer processing of data in background without blocking networking or handling inputs for the whole node. Another benefit of
...
void actor_thread_execute(actor_thread_t *thread) {
    ...
    actor_event_t event = {.producer = actor_thread_get_root_actor(thread), .type = ACTOR_EVENT_THREAD_ALARM};

    while (true) {
        actor_event_status_t previous_event_status = event.status;

        // route event to its listeners
        actor_thread_event_dispatch(thread, &event);

        // keep event for later processing if needed
        deferred_count += actor_thread_event_requeue(thread, &event, previous_event_status);

        // move to the next event
        if (!actor_thread_event_queue_shift(thread, &event, deferred_count)) {
            // wait for external notifications or scheduled wakeup
            actor_thread_event_await(thread, &event);
        }
    }
}
...
*/
static inline bool actor_thread_should_notify_actor(actor_thread_t *thread, actor_event_t *event, actor_t *actor, actor_worker_t *worker) {
    switch (event->type) {
    // Ticks get notified when thread starts and stops, so they can construct/destruct or schedule a periodical alarm
    case ACTOR_EVENT_THREAD_STOP:
        return true;
...
static actor_signal_t actor_thread_event_dispatch(actor_thread_t *thread, actor_event_t *event) {
    ...
    for (size_t i = 0; i < thread->worker_count; i++) {
        ...
        actor_worker_t *worker = event->consumer ? actor_thread_worker_for(thread, event->consumer) : &thread->workers[i];

        signal = actor_thread_event_actor_dispatch(thread, event, worker->actor, worker);
...
static actor_signal_t actor_thread_event_actor_dispatch(actor_thread_t *thread, actor_event_t *event, actor_t *actor, actor_worker_t *worker) {
    ...
    if (actor_thread_should_notify_actor(thread, event, actor, worker)) {
        ...
        if (event->status == ACTOR_EVENT_WAITING) {
            event->status = ACTOR_EVENT_RECEIVED;
        }
    }
...
static size_t actor_thread_event_requeue(actor_thread_t *thread, actor_event_t *event, actor_event_status_t previous_status) {
    ...
    case ACTOR_EVENT_ADDRESSED:
        ...
        if (queue != NULL) {
            ...
            if (queue == thread->queue) {
                event->status = ACTOR_EVENT_DEFERRED;
            } else {
                ...
                event->status = ACTOR_EVENT_WAITING;
            }

            // If the target queue is full, event gets lost.
            if (xQueueSend(queue, event, 0)) {
                if (event->status == ACTOR_EVENT_DEFERRED && previous_status != ACTOR_EVENT_DEFERRED) {
                    return 1;
                }
            } else {
    ...
    case ACTOR_EVENT_HANDLED:
        if (previous_status == ACTOR_EVENT_DEFERRED && thread->queue) {
            return -1;
        }
        break;
    default:
...
/*
  ...
  - A actor that was previously busy can send a `ACTOR_SIGNAL_CATCHUP` notification, indicating that it is ready to catch up with events
...
actor_signal_t actor_thread_event_await(actor_thread_t *thread, actor_event_t *event) {
    ...
    for (bool stop = false; stop == false;) {
        ...
        case ACTOR_SIGNAL_OK:
            ...
            *event = (actor_event_t){.producer = actor_thread_get_root_actor(thread), .type = ACTOR_EVENT_THREAD_ALARM};
        ...
        default:
            ...
            if (notification > 50) {
                if (thread->queue != NULL) {
                    ...
                    memcpy(event, (void *)notification, sizeof(actor_event_t));
                }
...
void actor_thread_worker_schedule(actor_thread_t *thread, actor_worker_t *worker, uint32_t time) {
    worker->next_time = time;
    actor_thread_schedule(thread, time);
}
        ...
        *thread_slot = thread;
        ...
        if (thread->worker_count > 0) {
            ...
            if (thread->workers) {
                ...
                if (thread->task != NULL) {
                    if (queue_size > 0) {
                        thread->queue = xQueueCreate(queue_size, sizeof(actor_event_t));
                    }
...
__attribute__((weak)) uint32_t actor_thread_iterate_workers(actor_thread_t *thread, actor_worker_t *destination) {
    return 0;
}
...
__attribute__((weak)) actor_thread_t *actor_thread_alloc(actor_t *actor) {
    return actor_malloc(sizeof(actor_thread_t));
}
