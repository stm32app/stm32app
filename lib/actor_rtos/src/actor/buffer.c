...
actor_buffer_t* actor_buffer_aligned_with_options(actor_t* owner,
  ...
  actor_buffer_t* buffer =
      actor_buffer_empty_with_options(owner, size + alignment - 1, options | ACTOR_BUFFER_ALIGNED);
...
actor_signal_t actor_buffer_reserve(actor_buffer_t* buffer, uint32_t size) {
  if (size > buffer->allocated_size) {
    ...
    actor_signal_t signal = actor_callback_buffer_allocation(buffer, size);
...
actor_signal_t actor_buffer_reserve_with_limits(actor_buffer_t* buffer,
                                                ...
                                                uint32_t block_size,
                                                uint32_t max_size) {
  uint32_t new_size = buffer->allocated_size ? buffer->allocated_size
                      : initial_size         ? initial_size
                                             : ACTOR_BUFFER_INITIAL_SIZE;
  if (max_size == 0)
...
actor_signal_t actor_buffer_read_from_ring_buffer(actor_buffer_t* buffer,
                                                  ...
                                                  uint16_t position) {
  //  Handle wraparound of circular buffer, copy memory into actor_buffer that can grow
  //  automatically
  if (position > previous_position) {
    return actor_buffer_append(buffer, &ring_buffer[previous_position],
                               position - previous_position);
  } else {
    actor_signal_t signal = actor_buffer_append(buffer, &ring_buffer[previous_position],
...
actor_signal_t actor_double_buffer_ingest_external_write(actor_buffer_t* back,
  ...
  if (actor_double_buffer_get_input(back, front) == back) {
    ...
    actor_signal_t signal = actor_buffer_read_from_ring_buffer(
        front, back->data, back->allocated_size, back->size, position);
    back->size = position;
    return signal;
  } else {
...
actor_buffer_t* actor_buffer_align(actor_buffer_t* buffer, uint8_t alignment) {
  uint8_t offset = alignment - ((uintptr_t)buffer->data) % alignment;  // fixme: 64 vs 32
  if (offset > 0) {
...
actor_buffer_t* actor_buffer_malloc(actor_t* actor) {
  actor_buffer_t* buffer = actor_malloc_region(ACTOR_REGION_FAST, sizeof(actor_buffer_t));
...
__attribute__((weak)) actor_buffer_t* actor_buffer_alloc(actor_t* actor) {
  return actor_malloc_region(ACTOR_REGION_FAST, sizeof(actor_buffer_t));
}
__attribute__((weak)) void actor_buffer_free(actor_buffer_t* buffer) {
  actor_free(buffer);
}

__attribute__((weak)) actor_signal_t actor_callback_buffer_allocation(actor_buffer_t* buffer,
                                                                      uint32_t size) {
