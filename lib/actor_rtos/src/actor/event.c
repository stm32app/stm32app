...
static void actor_message_set_status(actor_t *actor, actor_message_t *event, actor_message_status_t status) {
...
actor_signal_t actor_message_accept_and_start_job_generic(actor_t *actor, actor_message_t *event, actor_job_t **job_slot,
    ...
    actor_signal_t signal = actor_message_accept_and_process_generic(actor, event, &job->inciting_message, ready_status, busy_status, NULL);
    if (signal == ACTOR_SIGNAL_OK) {
        job->actor = actor;
        job->thread = thread;
        job->handler = handler;
        job->pointer = job_slot;
        debug_printf("| ├ Task start\t\t%s for %s\t\n", actor_stringify(actor), actor_thread_get_name(thread));
...
actor_signal_t actor_message_accept_and_pass_to_job_generic(actor_t *actor, actor_message_t *event, actor_job_t *job, actor_thread_t *thread,
    ...
    actor_signal_t signal = actor_message_accept_and_process_generic(actor, event, &job->incoming_message, ready_status, busy_status, NULL);
    if (signal == ACTOR_SIGNAL_OK) {
        actor_job_execute(job, ACTOR_SIGNAL_OK, event->producer);
    }
    return signal;
...
actor_signal_t actor_publish_message_generic(actor_t *actor, actor_message_type_t type, actor_t *target, uint8_t *data, uint32_t size,
                                           void *argument) {
    actor_prepared_message =
        (actor_message_t){.type = type, .data = data, .size = size, .argument = argument, .consumer = target, .producer = actor};
...
__attribute__((weak)) void actor_callback_message_handled(actor_t *actor, actor_message_t *event, actor_signal_t signal) {
}