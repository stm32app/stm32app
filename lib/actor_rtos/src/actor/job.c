...
actor_signal_t actor_job_execute(actor_job_t *job, actor_signal_t signal, actor_t *caller) {
    ...
    debug_printf("| ├ Task \t\t#%s at %u:%u for %s\n", actor_message_stringify(&job->inciting_message), job->job_phase, job->task_phase,
    ...
    for (bool halt = false; !halt;) {
        ...
        if (returned_signal < 0 || job_signal == ACTOR_SIGNAL_JOB_COMPLETE) {
            ...
            debug_printf("│ │ ├ Task complete\t%u:%u\n", job->job_phase, job->task_phase);
...
actor_signal_t actor_job_switch_thread(actor_job_t *job, actor_thread_t *thread) {
    job->thread = thread;
    if (actor_thread_is_interrupted(thread) /* || actor_thread_get_current(thread->actor) != thread*/) {
