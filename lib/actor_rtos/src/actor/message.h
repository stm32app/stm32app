...
/* Singnal is a numeric message, often used as a return code or status */
enum actor_signal
{
  ACTOR_SIGNAL_OK = 0,
  ACTOR_SIGNAL_REPEAT,
...
specific consumer, or broadcasted agents accepting that event type. A dispatched method can be
"taken" by agent so that no other agents will receieve it. Once message is processed, a consumer
will signal the producer about completion via a hook, so that producer can release the resources.
Event struct itself is copied into the queue directly, so it does not have to be allocated. When
consumer recieves an event, it may make a copy. Buffers on the other end are not copied, as they are
handled by reference. Messages have extra argument pointer, that can be used for custom purposes,
and have status that is used to establish if event was consumed or not.*/
...
/* Consume event if not busy, otherwise keep it enqueued for later without allowing others to take
 * it  */
#define actor_message_handle(actor, event, destination)                                      \
  actor_message_accept_and_process_generic(actor, event, destination, ACTOR_MESSAGE_HANDLED, \
                                           ACTOR_MESSAGE_DEFERRED, NULL)
/* Consume event with a given handler  if not busy, otherwise keep it enqueued for later without
 * allowing others to take it  */
#define actor_message_handle_and_process(actor, event, destination, handler)                 \
  actor_message_accept_and_process_generic(actor, event, destination, ACTOR_MESSAGE_HANDLED, \
                                           ACTOR_MESSAGE_DEFERRED, (actor_on_message_t)handler)
/* Consume event by starting a task if not busy, otherwise keep it enqueued for later without
 * allowing others to take it  */
#define actor_message_handle_and_start_job(actor, event, task, thread, handler)   \
  actor_message_accept_and_start_job_generic(actor, event, task, thread, handler, \
                                             ACTOR_MESSAGE_HANDLED, ACTOR_MESSAGE_DEFERRED)
/* Consume event with a running task if not busy, otherwise keep it enqueued for later without
 * allowing others to take it  */
#define actor_message_handle_and_pass_to_job(actor, event, task, thread, handler)   \
  actor_message_accept_and_pass_to_job_generic(actor, event, task, thread, handler, \
                                               ACTOR_MESSAGE_HANDLED, ACTOR_MESSAGE_DEFERRED)
...
#define actor_message_accept(actor, event, destination)                                      \
  actor_message_accept_and_process_generic(actor, event, destination, ACTOR_MESSAGE_HANDLED, \
                                           ACTOR_MESSAGE_ADDRESSED, NULL)
/* Consume event with a given handler if not busy, otherwise keep it enqueued for later unless other
 * actors take it first */
#define actor_message_accept_and_process(actor, event, destination, handler)                 \
  actor_message_accept_and_process_generic(actor, event, destination, ACTOR_MESSAGE_HANDLED, \
                                           ACTOR_MESSAGE_ADDRESSED, (actor_message_t)handler)
/* Consume event by starting a task if not busy, otherwise keep it enqueued for later unless other
 * actors take it first  */
#define actor_message_accept_and_start_job(actor, event, task, thread, handler)   \
  actor_message_accept_and_start_job_generic(actor, event, task, thread, handler, \
                                             ACTOR_MESSAGE_HANDLED, ACTOR_MESSAGE_ADDRESSED)
/* Consume event with a running task  if not busy, otherwise keep it enqueued for later unless other
 * actors take it first */
...
#define actor_message_receive(actor, event, destination)                                      \
  actor_message_accept_and_process_generic(actor, event, destination, ACTOR_MESSAGE_RECEIVED, \
                                           ACTOR_MESSAGE_RECEIVED, NULL)
...
#define actor_message_receive_and_process(actor, event, destination, handler)                 \
  actor_message_accept_and_process_generic(actor, event, destination, ACTOR_MESSAGE_RECEIVED, \
                                           ACTOR_MESSAGE_RECEIVED, (actor_message_t)handler)
...
#define actor_message_receive_and_start_job(actor, event, task, thread, handler)  \
  actor_message_accept_and_start_job_generic(actor, event, task, thread, handler, \
                                             ACTOR_MESSAGE_RECEIVED, ACTOR_MESSAGE_RECEIVED)
...
actor_signal_t actor_publish_message_generic(actor_t* actor,
                                             ...
                                             uint32_t size,
                                             void* argument);
#define actor_publish_message(actor, type) \
  actor_publish_message_generic(actor, type, NULL, NULL, 0, NULL)
#define actor_publish_message_for(actor, type, target) \
  actor_publish_message_generic(actor, type, target, NULL, 0, NULL)
#define actor_publish_message_with_data(actor, type, data, size) \
  actor_publish_message_generic(actor, type, NULL, data, size, NULL)
...
#define actor_publish_message_with_argument(actor, type, data, size, arg) \
  actor_publish_message_generic(actor, type, NULL, data, size, arg)
...
#define actor_job_publish_message(job, type) \
  actor_job_publish_message_generic(job, type, NULL, NULL, 0, NULL)
#define actor_job_publish_message_for(job, type, target) \
  actor_job_publish_message_generic(job, type, target, NULL, 0, NULL)
#define actor_job_publish_message_with_data(job, type, data, size) \
  actor_job_publish_message_generic(job, type, NULL, data, size, NULL)
...
#define actor_job_publish_message_with_argument(job, type, data, size, arg) \
  actor_job_publish_message_generic(job, type, NULL, data, size, arg)
