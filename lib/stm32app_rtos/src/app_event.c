...
app_signal_t actor_event_accept_and_start_job_generic(actor_t *actor, app_event_t *event, app_job_t **job_slot, app_thread_t *thread,
    ...
    if (signal == APP_SIGNAL_OK) {
        ...
        job->thread = thread;
        job->handler = handler;
        debug_printf("| ├ Task start\t\t%s for %s\t\n", app_actor_stringify(actor), app_thread_get_name(thread));
...
app_signal_t app_job_publish_event_generic(app_job_t *job, app_event_type_t type, actor_t *target, uint8_t *data, uint32_t size,
                                         void *argument) {
    job->outgoing_event =
        (app_event_t){.type = type, .data = data, .size = size, .argument = argument, .consumer = target, .producer = job->actor};

    actor_publish(job->actor, &job->outgoing_event);
    return app_job_wait(job);
}
...
app_signal_t actor_publish_event_generic(actor_t *actor, app_event_type_t type, actor_t *target, uint8_t *data, uint32_t size,
    ...
    return actor_publish(actor, &app_prepared_event);
}
...
app_signal_t actor_event_finalize(actor_t *actor, app_event_t *event) {
    if (event != NULL && event->type != APP_EVENT_IDLE) {
        if (event->type != APP_EVENT_THREAD_ALARM) {
            ...
            app_event_report_callback(actor, event);
...
__attribute__((weak)) void app_event_report_callback(actor_t *actor, app_event_t *event) {
