...
static actor_signal_t database_construct(actor_database_t *database) {
    actor_message_subscribe(database->actor, ACTOR_MESSAGE_START);
...
static actor_signal_t database_job_query(actor_job_t *job) {
    return 0;
}

static actor_signal_t database_job_connect(actor_job_t *job) {
...
static actor_signal_t database_phase_changed(actor_database_t *database, actor_phase_t phase) {
    return 0;
}

static actor_signal_t database_signal_received(actor_database_t *database, actor_signal_t signal, actor_t *caller, void *source) {
    return 0;
}

static actor_signal_t database_worker_sql(actor_database_t *database, actor_message_t *event, actor_worker_t *tick, actor_thread_t *thread) {
    return actor_job_execute_if_running_in_thread(database->job, thread);
}

static actor_signal_t database_worker_on_input(actor_database_t *database, actor_message_t *event, actor_worker_t *tick, actor_thread_t *thread) {
    switch (event->type) {
    case ACTOR_MESSAGE_START:
        debug_log_inhibited = false;
        return actor_message_handle_and_start_job(database->actor, event, &database->job, database->thread, database_job_connect);
    case ACTOR_MESSAGE_QUERY:
        return actor_message_handle_and_start_job(database->actor, event, &database->job, database->thread, database_job_query);
    default:
        break;
    }
    return 0;
...
static actor_worker_callback_t database_worker_assignment(actor_database_t *database, actor_thread_t *thread) {
...
static actor_signal_t database_phase_changed(actor_database_t *database, actor_phase_t phase) {
    ...
    case ACTOR_PHASE_CONSTRUCTION:
        return database_construct(database);
    case ACTOR_PHASE_LINKAGE:
        return database_link(database);
    case ACTOR_PHASE_START:
        return database_start(database);
    case ACTOR_PHASE_STOP:
        return database_stop(database);
    default:
        break;
    }
    return 0;
...
actor_interface_t actor_database_class = {
    .type = ACTOR_DATABASE,
    .size = sizeof(actor_database_t),
    .phase_subindex = ACTOR_DATABASE_PHASE,
    .validate = (actor_method_t)database_validate,
    .phase_changed = (actor_phase_changed_t)  database_phase_changed,
    .signal_received = (actor_signal_received_t)database_signal_received,
    .worker_assignment = (actor_worker_assignment_t)database_worker_assignment,
};
