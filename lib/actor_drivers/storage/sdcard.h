...
typedef struct storage_sdcard_properties {
    ...
    bool high_capacity; // 1 for SDHC/SDXC card 
...
static inline void storage_sdcard_set_high_capacity(storage_sdcard_t *sdcard, bool value) { 
    actor_set_property_numeric(sdcard->actor, STORAGE_SDCARD_HIGH_CAPACITY, (uint32_t)(value), sizeof(bool));
}
static inline bool storage_sdcard_get_high_capacity(storage_sdcard_t *sdcard) {
    return *((bool *) actor_get_property_pointer(sdcard->actor, STORAGE_SDCARD_HIGH_CAPACITY));
}
