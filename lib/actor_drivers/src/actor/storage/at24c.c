...
static actor_signal_t at24c_signal_received(storage_at24c_t *at24c, actor_signal_t signal, actor_t *caller, void *source) {
    return 0;
}

static actor_signal_t at24c_task_read(actor_job_t *job, actor_signal_t signal, actor_t *caller, uint8_t address, uint8_t *data, uint32_t size) {
    ...
    if (signal > ACTOR_SIGNAL_FAILURE) {
        ...
        case 1:
            ...
            actor_publish(job->actor, &((actor_message_t){
                                            ...
                                            .size = bytes_on_page,
                                            .argument = i2c_pack_message_argument(at24c->properties->i2c_address, address + job->counter),
                                        }));
...
static actor_signal_t at24c_task_write(actor_job_t *job, actor_signal_t signal, actor_t *caller, uint8_t address, uint8_t *data, uint32_t size) {
    storage_at24c_t *at24c = job->actor->object;
    uint32_t bytes_on_page;
...
static actor_signal_t at24c_job_diagnose(actor_job_t *job, actor_signal_t signal, actor_t *caller) {
    ...
    case 0:
        return at24c_task_read(job, signal, caller, 0x0010, NULL, 2);
    case 1:
        return at24c_task_write(job, signal, caller,0x0010,
                                &((uint8_t[]){
    ...
    case 3:
        ok = (job->output_buffer->data[0] + 1 == at24c->target_buffer->data[0] && job->output_buffer->data[1] == at24c->target_buffer->data[1] &&
              job->output_buffer->size == at24c->target_buffer->size);

        if (ok) {
            debug_printf("  - Diagnostics OK: 0x%x %s\n", actor_index(job->actor), actor_stringify(job->actor));
            signal = ACTOR_SIGNAL_TASK_COMPLETE;
        } else {
            debug_printf("  - Diagnostics ERROR: 0x%x %s\n", actor_index(job->actor), actor_stringify(job->actor));
            signal = ACTOR_SIGNAL_FAILURE;
        }
        break;
...
static actor_signal_t at24c_worker_input(storage_at24c_t *at24c, actor_message_t *event, actor_worker_t *tick, actor_thread_t *thread) {
    switch (event->type) {
    case ACTOR_MESSAGE_DIAGNOSE:
        return actor_message_receive_and_start_job(at24c->actor, event, &at24c->job, at24c->actor->node->low_priority,
                                                  at24c_job_diagnose);
    ...
    case ACTOR_MESSAGE_RESPONSE:
        return actor_message_handle_and_pass_to_job(at24c->actor, event, at24c->job, at24c->job->thread, at24c->job->handler);
...
static actor_signal_t at24c_worker_high_priority(storage_at24c_t *at24c, actor_message_t *event, actor_worker_t *tick, actor_thread_t *thread) {
    return actor_job_execute_if_running_in_thread(at24c->job, thread);
}
static actor_signal_t at24c_worker_low_priority(storage_at24c_t *at24c, actor_message_t *event, actor_worker_t *tick, actor_thread_t *thread) {
    return actor_job_execute_if_running_in_thread(at24c->job, thread);
}

static actor_signal_t at24c_message_handled(storage_at24c_t *at24c, actor_message_t *event, actor_signal_t signal) {
    switch (event->type) {
    case ACTOR_MESSAGE_WRITE:
...
static actor_worker_callback_t at24c_worker_assignment(storage_at24c_t *at24c, actor_thread_t *thread) {
