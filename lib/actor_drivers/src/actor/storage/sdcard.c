...
static actor_signal_t sdcard_property_after_change(storage_sdcard_t *sdcard, uint8_t index, void *value, void *old) {
    switch (index) {
...
static int sdcard_lfs_read(const struct lfs_config *c, lfs_block_t block, lfs_off_t off, void *buffer, lfs_size_t size) {
    ...
    actor_job_publish_message_with_argument_for(sdcard->job, ACTOR_MESSAGE_READ_TO_BUFFER, sdcard->sdio, buffer,
                                                size / sdcard->properties->block_size,
                                                (void *)(block + off / sdcard->properties->block_size));
    return 0;
}

static int sdcard_lfs_prog(const struct lfs_config *c, lfs_block_t block, lfs_off_t off, const void *buffer, lfs_size_t size) {
    storage_sdcard_t *sdcard = c->context;
    actor_job_publish_message_with_argument_for(sdcard->job, ACTOR_MESSAGE_WRITE, sdcard->sdio, (void *)buffer,
                                                size / sdcard->properties->block_size,
                                                (void *)(block + off / sdcard->properties->block_size));
    return 0;
}
...
static int sdcard_lfs_erase(const struct lfs_config *c, lfs_block_t block) {
    // actor_publish_message_with_argument_for(sdcard->actor, ACTOR_MESSAGE_ERASE, sdcard->sdio->actor, NULL, 1,
    //                                      (void *)(block));
    // coru_yield();
    return 0;
}
...
static actor_signal_t sdcard_message_handled(storage_sdcard_t *sdcard, actor_message_t *event, actor_signal_t signal) {
    actor_thread_actor_wakeup(sdcard->job->thread, sdcard->actor);
    return 0;
}

static actor_signal_t sdcard_signal_received(storage_sdcard_t *sdcard, actor_signal_t signal, actor_t *caller, void *source) {
    switch (signal) {
    default:
        break;
    }
...
static actor_signal_t sdcard_job_read(actor_job_t *job, actor_signal_t signal, actor_t *caller) {
    ...
    actor_buffer_t *buffer = actor_buffer_target_aligned(job->actor, job->inciting_message.data, job->inciting_message.size, 16);
...
static actor_signal_t sdcard_job_write(actor_job_t *job, actor_signal_t signal, actor_t *caller) {
    ...
    actor_buffer_t *buffer = actor_buffer_source(job->actor, job->inciting_message.data, job->inciting_message.size);
...
static actor_signal_t sdcard_worker_thread(storage_sdcard_t *sdcard, actor_message_t *event, actor_worker_t *tick, actor_thread_t *thread) {
    actor_job_execute_if_running_in_thread(sdcard->job, thread);
    return 0;
}

static actor_signal_t sdcard_worker_on_input(storage_sdcard_t *sdcard, actor_message_t *event, actor_worker_t *tick,
                                             actor_thread_t *thread) {
    switch (event->type) {
    case ACTOR_MESSAGE_THREAD_ALARM:
        debug_log_inhibited = false;
        return actor_message_handle_and_start_job(sdcard->actor, event, &sdcard->job, sdcard->thread, sdcard_job_mount);
    case ACTOR_MESSAGE_READ_TO_BUFFER:
    case ACTOR_MESSAGE_READ:
        return actor_message_handle_and_start_job(sdcard->actor, event, &sdcard->job, sdcard->thread, sdcard_job_read);
    case ACTOR_MESSAGE_WRITE:
        return actor_message_handle_and_start_job(sdcard->actor, event, &sdcard->job, sdcard->thread, sdcard_job_write);
    case ACTOR_MESSAGE_OPEN:
        return actor_message_handle_and_start_job(sdcard->actor, event, &sdcard->job, sdcard->thread, sdcard_job_open);
    case ACTOR_MESSAGE_CLOSE:
        return actor_message_handle_and_start_job(sdcard->actor, event, &sdcard->job, sdcard->thread, sdcard_job_close);
    case ACTOR_MESSAGE_ERASE:
        return actor_message_handle_and_start_job(sdcard->actor, event, &sdcard->job, sdcard->thread, sdcard_job_erase);
...
static actor_worker_callback_t sdcard_worker_assignment(storage_sdcard_t *sdcard, actor_thread_t *thread) {
