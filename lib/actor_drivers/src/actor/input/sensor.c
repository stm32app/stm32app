...
static actor_signal_t sensor_receive(input_sensor_t *sensor, actor_t *actor, void *value, void *channel) {
    (void)channel; /*unused*/
...
static actor_signal_t sensor_phase(input_sensor_t *sensor, actor_phase_t phase) {
    (void)sensor;
    (void)phase;
    return 0;
}
...
actor_class_t input_sensor_class = {
    ...
    .construct = (actor_method_t)sensor_construct,
    .link = (actor_method_t)sensor_link,
    .start = (actor_method_t)sensor_start,
    .stop = (actor_method_t)sensor_stop,
