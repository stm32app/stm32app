...
static actor_signal_t epaper_set_busy_phase(screen_epaper_t *epaper) {
    if (actor_gpio_get(epaper->properties->busy_port, epaper->properties->busy_pin) == 1) {
        ...
        actor_set_phase(epaper->actor, ACTOR_PHASE_OPERATION);
    }
    return 0;
}
...
static actor_signal_t epaper_set_resetting_phase(screen_epaper_t *epaper) {
    ...
    case 3:
        ...
        //actor_set_temporary_phase(epaper->actor, ACTOR_RESETTING, 200000);
        break;
    case 4: epaper->resetting_phase = 0; actor_set_phase(epaper->actor, ACTOR_PHASE_OPERATION);
    }
    return 0;
}
...
actor_interface_t screen_epaper_class = {
    .type = SCREEN_EPAPER,
    .size = sizeof(screen_epaper_t),
    .phase_subindex = SCREEN_EPAPER_PHASE,
    .validate = (actor_method_t) epaper_validate,
    .construct = epaper_construct,
    .destruct =  epaper_destruct,
    .start =  epaper_start,
    .stop =  epaper_stop,
    .link =  epaper_link,
    .phase_changed = (actor_phase_changed_t)  epaper_phase,
