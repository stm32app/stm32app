...
static ODR_t ili9341_property_after_change(screen_ili9341_t* ili9341,
                                           uint8_t index,
                                           void* value,
                                           void* old) {
  switch (index) {
    ...
    case SCREEN_ILI9341_INVERT_COLORS:
      ili9341_send_command(ili9341, ili9341->properties->invert_colors
                                        ? ILI9341_CMD_INVERT_COLORS_ON
                                        : ILI9341_CMD_INVERT_COLORS_OFF);
      break;
    default:
      break;
  }
...
static struct {
  ...
  uint16_t pins;
    ...
    {GPIOD, GPIO0 | GPIO1 | GPIO4 | GPIO5 | GPIO8 | GPIO9 | GPIO10 | GPIO11 | GPIO12 | GPIO13 |
                GPIO14 | GPIO15},
    {GPIOE,
     GPIO0 | GPIO1 | GPIO7 | GPIO8 | GPIO9 | GPIO10 | GPIO11 | GPIO12 | GPIO13 | GPIO14 | GPIO15},
...
static actor_signal_t ili9341_signal_received(screen_ili9341_t* ili9341,
...
static actor_signal_t ili9341_task_sleep(actor_job_t* job,
                                         actor_signal_t signal,
                                         actor_t* caller,
                                         bool arg) {
  screen_ili9341_t* ili9341 = job->actor->object;
  actor_async_task_begin();
  actor_async_await(actor_mutex_acquire(&ili9341->sleep_mutex, job));
  ...
  actor_async_await(actor_job_delay(job, 5));
  actor_async_task_end();
}

static actor_signal_t ili9341_task_apply(actor_job_t* job, actor_signal_t signal, actor_t* caller) {
  screen_ili9341_t* ili9341 = job->actor->object;
  actor_async_task_begin();
  ...
  screen_ili9341_set_invert_colors(ili9341->actor,
                                   screen_ili9341_get_invert_colors(ili9341->actor));
  actor_async_task_end();
}

static actor_signal_t ili9341_job_initialize(actor_job_t* job,
                                             actor_signal_t signal,
                                             actor_t* caller) {
  actor_async_task_begin();
  actor_async_await(ili9341_task_configure(job, signal, caller));
  actor_async_await(ili9341_task_sleep(job, signal, caller, false));
  actor_async_await(ili9341_task_apply(job, signal, caller));
  actor_async_task_end();
}

static actor_signal_t ili9341_on_input(screen_ili9341_t* ili9341,
                                       actor_message_t* event,
                                       actor_worker_t* worker,
                                       actor_thread_t* thread) {
  switch (event->type) {
    case ACTOR_MESSAGE_THREAD_START:
      actor_message_receive_and_start_job(ili9341->actor, event, &ili9341->job,
                                          ili9341->actor->node->high_priority,
                                          ili9341_job_initialize);
      break;
    default:
      break;
  }
...
static actor_signal_t ili9341_high_priority(screen_ili9341_t* ili9341,
                                            actor_message_t* event,
                                            actor_worker_t* worker,
                                            actor_thread_t* thread) {
  actor_mutex_attempt_expiration(&ili9341->sleep_mutex, ili9341->job,
                                 &ili9341->sleep_mutex_expiration_time);
  actor_job_execute_if_running_in_thread(ili9341->job, thread);
  return 0;
}

static actor_worker_callback_t ili9341_worker_assignment(screen_ili9341_t* ili9341,
                                                         actor_thread_t* thread) {
